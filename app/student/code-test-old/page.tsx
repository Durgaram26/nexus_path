...
const getMonacoLanguage = (languageName: string): string => {
  ...
  const mappedLanguage = languageMap[languageName] || 'python';
  console.log('Monaco Language Mapping:', { languageName, mappedLanguage });
...
export default function CodeTestPage() {
  ...
  const loadDefaultQuestions = async () => {
    try {
      ...
      toast.info('AI is generating your personalized coding questions...');
      
      // Get parameters from URL
      const urlParams = new URLSearchParams(window.location.search);
      const difficulty = urlParams.get('difficulty') || 'medium';
      const language = urlParams.get('language') || 'python';
      ...
      const languageMap: { [key: string]: { name: string, id: number } } = {
      ...
      const response = await fetch('/api/code-execution/generate-questions', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (generatedQuestions.length > 0) {
          ...
          setSelectedLanguage({ id: selectedLanguage.id, name: selectedLanguage.name });
          setCode(generatedQuestions[0]?.starterCode || '');
          setLoading(false);
          setIsGeneratingAI(false);
          setShowFullscreenPrompt(true);
          toast.success(`AI-generated ${difficulty} ${selectedLanguage.name} questions loaded successfully!`);
        } else {
  ...
  const loadFallbackQuestions = () => {
    // No fallback questions - show error message
    setLoading(false);
  ...
  const enterFullscreen = () => {
    if (document.documentElement.requestFullscreen) {
      ...
      setTestStarted(true);
      toast.success('Fullscreen mode activated! Test started!');
    }
  };
  ...
  useEffect(() => {
    ...
    // If no questions are received within 5 seconds, load default questions
  ...
  }, [testStarted, testCompleted, questions.length]);
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        if (result.status.id > 2) {
          ...
          try {
            await fetch('/api/student/code-execution-history', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
  ...
  const nextQuestion = async () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      // Notify parent window that test is completed
      if (window.opener) {
        ...
        <div className="text-center max-w-md mx-auto">
          {isGeneratingAI ? (
            <>
              ...
              <h2 className="text-2xl font-bold text-gray-800 mb-4">AI Generating Questions</h2>
              <div className="space-y-3 text-left">
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-blue-600 rounded-full animate-pulse"></div>
                  <span className="text-gray-700">Analyzing your coding profile...</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-blue-600 rounded-full animate-pulse" style={{animationDelay: '0.5s'}}></div>
                  <span className="text-gray-700">Generating personalized questions...</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-blue-600 rounded-full animate-pulse" style={{animationDelay: '1s'}}></div>
                  <span className="text-gray-700">Creating test cases and solutions...</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-blue-600 rounded-full animate-pulse" style={{animationDelay: '1.5s'}}></div>
                  <span className="text-gray-700">Finalizing your coding challenge...</span>
                </div>
              </div>
  ...
  // Show fullscreen prompt before starting test
  if (showFullscreenPrompt) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <Card className="max-w-2xl w-full">
          <CardHeader>
            <div className="text-center">Test Instructions</div>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold mb-2">Coding Test</h2>
              <p className="text-gray-600">AI-generated programming challenges with test cases</p>
            </div>
            ...
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              ...
              <ul className="text-sm text-yellow-700 space-y-1">
                <li>• Solve all programming challenges to complete the test</li>
                <li>• You can navigate between questions</li>
                <li>• Test will auto-submit when time expires</li>
                <li>• Use hints if you get stuck</li>
                <li>• <strong>Fullscreen mode is mandatory</strong> - test will not start without it</li>
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-gray-500 mt-2">
                Test will automatically start when fullscreen mode is activated
              </p>
            </div>
          </CardContent>
        </Card>
